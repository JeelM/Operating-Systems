...
void insertSJF(struct node **head, Task *newTask);
void schedule(){
    ...
    if(*head == NULL){
        *head = newNode;
        ...
        while(currentNode != NULL){
            if(newNode->task->burst < currentNode->task->burst && currentNode == *head){
                newNode->next = currentNode;
                *head = newNode;
                break; 
            }
            else if(newNode->task->burst >= currentNode->task->burst && currentNode->next == NULL){
                currentNode->next = newNode;
                break;
            }
            else if(newNode->task->burst >= currentNode->task->burst && newNode->task->burst < currentNode->next->task->burst){
